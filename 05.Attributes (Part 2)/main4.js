(function() {
  ...
  const vertexShaderSource = (
    ...
    layout(location = 1) in mediump uint aPointSize;
    layout(location = 2) in mediump uvec3 aColor;
    out vec4 vColor;
    void main()
    {
      gl_Position = vec4(aCoords, 0.0, 1.0);
      gl_PointSize = float(aPointSize);
      vColor = vec4(vec3(aColor)/255.0, 1.0);
    }`
  );
  ...
  gl.vertexAttribI4ui(aPointSizeLocation, 50, 0, 0, 0);
  gl.vertexAttribI4ui(aColorLocation, 255, 0, 255, 0);
  ...
  gl.vertexAttribIPointer(aPointSizeLocation, 1, gl.UNSIGNED_BYTE, 1, 0);
  ...
  gl.vertexAttribIPointer(aColorLocation, 3, gl.UNSIGNED_BYTE, 3, 0);
